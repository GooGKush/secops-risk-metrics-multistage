...
#
# Generates references/metrics-catalog.md directly from scripts/preflight_validator.py
# ensuring Code-as-Single-Source-of-Truth.
...
from scripts.preflight_validator import METRIC_CATALOG, EntityType

CLOUD_AUDIT_DISTRIBUTION = [
    ("GENERIC_EVENT", 30999, "Control plane & routine cloud operations", "Broad background automation, service-to-service calls"),
    ("RESOURCE_WRITTEN", 2575, "System/generic resource modifications", "High-volume writes, data store ingestion surges"),
...
def generate_metrics_catalog() -> str:
  lines = [
  ...
  for evt, vol, role, focus in CLOUD_AUDIT_DISTRIBUTION:
    lines.append(f"| `{evt}` | {vol:,} | {role} | {focus} |")
...
def main():
  catalog_md = generate_metrics_catalog()
  output_path = REPO_DIR / "references" / "metrics-catalog.md"
  output_path.write_text(catalog_md, encoding="utf-8")
  print(f"Generated {output_path} ({len(catalog_md.encode('utf-8'))} bytes)")

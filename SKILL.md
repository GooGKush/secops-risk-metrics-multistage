...
3. **Visualization Strategy (Dual-Surface Context-Aware Architecture)**:
   - **Surface 1: Interactive UI Chat (MANDATORY GRAPHICAL SVG RADAR)**: In Pillar 1, save SVG artifact via `scripts/radar_collector.py --format svg --output <artifact_dir>/radar_<entity>.svg` and embed inline via `![360° Behavioral Risk Radar: <entity>](file:///<artifact_dir>/radar_<entity>.svg)` (or `<agent-embed>`). Include CommonMark table with Unicode magnitude progress bars (`▰▰▰▰▱▱▱▱`). NEVER substitute ASCII art for SVG in UI sessions (CRITICAL VISUAL SPECIFICATION VIOLATION).
   - **Surface 2: Cases & Headless CLI (ASCII Fallback)**: Use ASCII radar card from `scripts/radar_collector.py --format ascii` SOLELY for SOAR case comments (`create_case_comment`) or headless CLI.
   - **Canonical 5-Spoke SVG Layout**: Center $(280, 240)$, max $r=150$. Rings: $r=37.5$ ($+1\sigma$), $75$ ($+2\sigma$), $112.5$ (`#d93025` $+3.0\sigma$), $150$ ($+4\sigma$). Spokes: Auth ($280,90$), Cloud ($423,194$), Workspace ($368,361$), Network ($192,361$), Endpoint ($137,194$). Polygon: `<polygon points="..." fill="rgba(26,115,232,0.25)" stroke="#1a73e8"/>` (red if $D \ge 3.0\sigma$). Layout `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 560 460">`.
   - **Sanitization Invariant**: NEVER output raw `<svg>` directly into markdown text (stripped by DOMPurify) and NEVER output raw base64 data-URIs in chat (blocked by CSP). Embed via `<agent-embed>` or file path.

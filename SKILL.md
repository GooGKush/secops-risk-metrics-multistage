...
* **Hard Stop on API Error (MANDATORY STOP — ZERO SILENT FALLBACK)**: If API query fails, STOP IMMEDIATELY. Simulating baselines is STRICTLY PROHIBITED.
...
* **Zero-Hallucination Compiler Grammar Contract**:
  ...
  - *Syntax Invariants*: No `in ("A", "B")` (use `%list`/`or`); no member dot-notation in `match:`; no `events:`; no `sqrt(...)` (use `$dist_sq`); no `by 24h` (use `by 1d`); `if(cond, then, else)` in `outcome:` requires 3 args; safe divisors via `$safe_std = if($std > 0, $std, 0.001)` or `+ 1.0`; enums in `references/clean-handoff-udm-schema.md`.

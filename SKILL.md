...
* **Zero-Hallucination Compiler Grammar Contract**:
  ...
  - *Syntax Invariants*: No `in ("A", "B")` (use `%list`/`or`); no member dot-notation in `match:`; no `events:`; no `sqrt(...)` (use `$dist_sq`); no `by 24h` (use `by 1d`); no `if(...)` or `count(if(...))` in outcome (no conditional aggregation; filter events above `match:`); dispersion floor `+ 1.0` in outcome divisors (`($obs - $avg) / ($std + 1.0)`); enums in `references/clean-handoff-udm-schema.md`.

...
3. **Visualization Strategy (Dual-Surface Context-Aware Architecture)**:
   - **Adaptive Capability Routing**:
     • *Jetski (`run_command` present)*: Execute `scripts/radar_collector.py --format dual --output <artifact_dir>/radar_<entity>.html`. In Pillar 1, output `<agent-embed src="file:///<artifact_dir>/radar_<entity>.html"></agent-embed>`, link `[Open 360° Radar](file:///<artifact_dir>/radar_<entity>.html)`, and CommonMark table with Unicode magnitude progress bars (`▰▰▰▰▱▱▱▱`). Jetski Web renders HTML canvas; Jetski CLI displays table and path.
     • *Generic Web MCP (no shell tool)*: In Pillar 1, emit CommonMark table with Unicode magnitude progress bars (`▰▰▰▰▱▱▱▱`) + pure inline `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 620 480">` directly in markdown (zero script calls).
     • *Terminal CLI (prompt asks for cli/ascii)*: Emit CommonMark table + ASCII horizontal radar card.
     • *Visual Invariant*: Never emit ```json_chart or blank placeholders. Omitting charts is a CRITICAL VISUAL SPECIFICATION VIOLATION.
   - **Canonical 5-Spoke SVG Layout**: Center $(310, 260)$, max $r=125$. Rings: $r=31.25$ ($+1\sigma$), $62.5$ ($+2\sigma$), $93.75$ (`#d93025` $+3.0\sigma$), $125$ ($+4\sigma$). Spokes: Auth ($310,135$), Cloud ($429,221$), Workspace ($383,361$), Network ($237,361$), DNS/Endpoint ($191,221$). Polygon: `<polygon points="..." fill="rgba(26,115,232,0.25)" stroke="#1a73e8" stroke-width="2"/>` (red if $D \ge 3.0\sigma$). Layout `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 620 480">`.
...
*Pre-Output Tool Execution Guard*: On clearance, execute `udm_search` across sectors. In Jetski, execute `scripts/radar_collector.py` FIRST to write the artifact to disk. In generic MCP clients (no shell tool), compute vector coordinates directly and emit pure inline `<svg>`. Zero `json_chart`.
The report **MUST STRICTLY CONTAIN ALL 6 NUMBERED PILLARS**:
1. **Statistical Outlier Report**: `[Target Metric]` ([Statistical Model]) with 30-day baseline (`window: 30d`). (For 360° Radar: In Jetski, save artifact via `scripts/radar_collector.py` and embed `<agent-embed src="file:///<artifact_dir>/radar_<entity>.html"></agent-embed>` with Unicode table. In generic MCP web clients, embed pure inline `<svg>` and Unicode table. In terminal CLI, embed ASCII radar card).

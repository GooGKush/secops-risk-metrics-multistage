...
3. **Visualization Strategy (Dual-Surface Context-Aware Architecture)**:
   - **Surface 1: Interactive UI Chat (MANDATORY GRAPHICAL SVG RADAR)**: In Pillar 1, save artifact via `scripts/radar_collector.py --format html --output <artifact_dir>/radar_<entity>.html` and embed inline via `<agent-embed src="file:///<artifact_dir>/radar_<entity>.html"></agent-embed>`. Include CommonMark table with Unicode magnitude progress bars (`▰▰▰▰▱▱▱▱`). NEVER substitute ASCII art for SVG in UI sessions (CRITICAL VISUAL SPECIFICATION VIOLATION).
...
1. **Statistical Outlier Report**: `[Target Metric]` ([Statistical Model]) with 30-day baseline (`window: 30d`). (For 360° Radar: **MANDATORY GRAPHICAL SVG RADAR** — Save to `<artifact_dir>/radar_<entity>.html` and embed inline via `<agent-embed src="file:///<path>">` along with CommonMark table and Unicode magnitude progress bars. ASCII is strictly for SOAR case comments).
